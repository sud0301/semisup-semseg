...
def get_arguments():
    ...
    parser.add_argument("--batch-size-unlab", type=int, default=BATCH_SIZE,
                        help="Number of images sent to the network in one step.")
    ...
    parser.add_argument("--eta-min-factor", type=float, default=10,
                        help="Base learning rate for training with polynomial decay.")
...
def main():
    ...
    if args.labeled_ratio is None:
        ...
        trainloader_remain = data.DataLoader(train_dataset,
                        batch_size=args.batch_size_unlab, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
        ...
        trainloader = data.DataLoader(train_dataset,
                        batch_size=args.batch_size, sampler=train_sampler, num_workers=4, pin_memory=True, drop_last=True)
        trainloader_remain = data.DataLoader(train_dataset,
                        batch_size=args.batch_size_unlab, sampler=train_remain_sampler, num_workers=4, pin_memory=True, drop_last=True)
        trainloader_gt = data.DataLoader(train_dataset,
                        batch_size=args.batch_size, sampler=train_gt_sampler, num_workers=4, pin_memory=True, drop_last=True)
    ...
    if args.dataset == 'pascal_voc':    
        valloader = data.DataLoader(VOCDataSet(args.data_dir, args.data_list, crop_size=(505, 505), mean=IMG_MEAN, scale=False, mirror=False),
                                        batch_size=1, shuffle=False, pin_memory=True)
        interp_val = nn.Upsample(size=(505, 505), mode='bilinear', align_corners=True)
    elif args.dataset == 'cityscapes':
        val_dataset = data_loader( data_path, img_size=(512, 1024), is_transform=True, split='val')
        valloader = data.DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, pin_memory=True)
    ...
    scheduler = CosineAnnealingLR(optimizer, T_max=args.num_steps, eta_min=args.learning_rate/args.eta_min_factor)
    optimizer.zero_grad()

    # optimizer for discriminator network
    optimizer_D = optim.Adam(model_D.parameters(), lr=args.learning_rate_D, betas=(0.9,0.99))
    scheduler_D = CosineAnnealingLR(optimizer_D, T_max=args.num_steps, eta_min=args.learning_rate_D/args.eta_min_factor)
    ...
    for i_iter in range(args.num_steps):
        ...
        if i_iter%10==0:
            ...
            args.writer.add_scalar('train/7.lr', optimizer.param_groups[0]['lr'], log_idx)
...
def validate(valloader, interp_val, model):
    ...
    miou_val = get_iou(args, data_list, args.num_classes, filename)
